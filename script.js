document.addEventListener('DOMContentLoaded', () => {
  ...
  function getCurrentPieFillColor() {
      if (isTimerGloballyPaused && !(isSequenceActive && pauseBtn.textContent === "다음 항목 시작")) { // "다음 항목 시작" 상태는 정지상태가 아님
  ...
  function updateDigitalDisplay() {
      ...
      if (isSequenceActive && currentSequenceIndex >= 0 && currentSequenceIndex < sequenceData.length) {
          ...
          titlePrefix = currentItem.name ? `${currentItem.name} (${currentSequenceIndex + 1}/${sequenceData.length}) - ` : `항목 ${currentSequenceIndex + 1}/${sequenceData.length} - `;
  ...
  function updateCurrentTimerVisuals() { 
      ...
      if (isSequenceActive && isTimerGloballyPaused && pauseBtn.textContent === "다음 항목 시작") { // 다음 항목 대기중일 때는 파이 안 그림
           setProgress(0);
      } else {
  ...
  function animationLoop() {
      ...
      if (currentRemainingMs <= 0) { // 타이머 종료 (단일 또는 시퀀스 아이템)
        ...
        const currentItemName =
            ...
            (sequenceData[currentSequenceIndex].name || `항목 ${currentSequenceIndex + 1}`) : "타이머";

        if (isSequenceActive) { // 시퀀스 아이템 종료
          currentSequenceDisplay.textContent = `${currentItemName} 완료!`;
          if (currentSequenceIndex >= sequenceData.length - 1) { // 모든 시퀀스 완료
            ...
            document.title = `${currentItemName} 완료 - 다음 대기`;
          }
        } else { // 단일 타이머 종료
          // setTimeout(alert('타이머 종료!'), 1000);
          startSound();
          document.title="⏰ 시간 종료! - 타이머";
          resetSingleTimer(); 
        }
        return;
      }
  ...
  function updateSequenceItemNumbers() { /* 이전과 동일 */ const items = sequenceItemsContainer.querySelectorAll('.sequence-item'); items.forEach((item, index) => { item.querySelector('.sequence-item-number').textContent = `항목 ${index + 1}`;});}
  ...
  function collectSequenceDataFromUI() {
      ...
      items.forEach(itemDiv => {
          ...
          if (duration > 0) {
              sequenceData.push({
                  name: name || `항목 ${sequenceData.length + 1}`,
                  duration,
  ...
  function startSequenceExecution() {
      ...
      if (sequenceData.length === 0) { alert("실행할 시퀀스 항목이 없습니다."); return; }
  ...
  function loadNextSequenceItem() {
      ...
      currentSequenceDisplay.textContent = `${item.name} (${currentSequenceIndex + 1}/${sequenceData.length}) 진행중...`;
  ...
  function stopSequenceExecution(completed = false) {
      ...
      if (completed) {
          currentSequenceDisplay.textContent = "🎉 모든 시퀀스 완료! 🎉";
      } else {
          currentSequenceDisplay.textContent = "시퀀스 중지됨.";
      }
  ...
  function updateMainControlsState() {
    ...
    const canStartNextSequenceItem =
      ...
      if (isGlobalSequenceModeEnabled) {
        ...
        collectSequenceDataFromUI();
        if (isSequenceActive) {
          startBtn.textContent = "재개"; // 또는 현재 상태에 따라
          startBtn.disabled = !isTimerGloballyPaused; // 일시정지 상태일 때만 재개 가능
          
          pauseBtn.textContent = canStartNextSequenceItem ? "다음 항목 시작" : "❚❚";
          ...
          currentSequenceDisplay.textContent = "시퀀스 준비 완료. '시퀀스 시작'을 누르세요.";
        }
      } else { // 단일 타이머 모드
  ...
  pauseBtn.addEventListener('click', () => {
      if (pauseBtn.textContent == "다음 항목 시작" && !pauseBtn.disabled || isGlobalSequenceModeEnabled && isSequenceActive && isTimerGloballyPaused && currentSequenceIndex < sequenceData.length -1 && !autoStartNextSequenceItem) {
          // "다음 항목 시작" 로직
          loadNextSequenceItem();
  ...
  toggleSettingsButton.addEventListener('click', function() {
      ...
      this.textContent = areSettingsVisible ? '설정 숨기기' : '설정 보이기';
  });
  ...
  function restoreOriginalDisplays() {
          ...
          fullscreenTimerBtn.textContent = "타이머 전체화면";
  }
  ...
  (() => {
      ...
      topControlsContainer.style.display = areSettingsVisible ? 'flex' : 'none'; 
      toggleSettingsButton.textContent = areSettingsVisible ? '설정 숨기기' : '설정 보이기';
